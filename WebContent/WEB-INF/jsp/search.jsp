...
<script>
	function like(id)
	{
		$.ajax({
			  ...
			  data: {"id": id },
			  statusCode: {
				  200: function() { alert("Obrigado por ajudar a comunidade Ouro Branquense!");},
				  202: function() { alert("Você já votou nessa opção!");}
				}
			});
	}
		...
		<div id="body">
				...
				<div class="container mx-auto col-md-8">
					<div class="row">
						<form class="container" action="search" method="post">
							<input class="form-control form-position" style="float:left" type="search" name="query"
			...
			<div class="border mx-auto col-md-8" style="padding: 10px">
				<c:if test="${fn:length(produtos) == 0}">
					<p>
					<h2 class="alert alert-danger">Nenhum produto encontrado para
						essas palavras-chave.</h2>

				</c:if>
				<c:if test="${fn:length(produtos) > 0}">
					<div class="alert alert-success text-center rounded">
						<h3 class="alert-heading"	>Sucesso! Existem ofertas para esse produto!</h3>
						<hr>
						<h6>Muitas ofertas duram apenas até acabar os estoques, por
							isso ofertas mais recentes tem mais chances de ainda estarem nos
							supermercados.</h6>
						<h6>Ajude a comuninade ouro branquense votando positivamente
							em ofertas que você sabe que estão ativas e negativamente em
							ofertas que já acabaram.</h6>
					</div>
					...
					<h6>Viu alguma oferta e gostaria de compartilhar ela com a
							...
							<input class="btn btn-outline-info" style ="display:inline-block" type="submit"
								value="Enviar Oferta" onClick="openModal()">
					...
					<table class="table table-condensed table-hover">
						...
						<tbody>
							<c:forEach items="${produtos}" var="produto">
								<tr>
									<td scope="row"><h6 class="centerV">${produto.nome}</h6></td>
									<td><h6 class="centerV">
											<fmt:formatNumber type="number" minFractionDigits="2"
												value="${produto.preco}" />
										</h6></td>
									<td><p class="centerV">${produto.quantidade}</p></td>
									<td><p class="centerV">${produto.mercado }</p></td>
									<td><c:if test="${produto.dias == 0}">
											<p class="centerV">Hoje</p>
										</c:if> <c:if test="${produto.dias > 0}">
											<p class="centerV">${produto.dias }dias</p>
										</c:if></td>
									<td><input class="buttonLike"
										onClick="like(${produto.id})" type="submit" value=""></input>
										<input class="buttonDislike" onClick="dislike(${produto.id})"
										type="submit" value=""></input></td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
			...
			<div>
				<form action= "" id="nome">
					...
					<input class="form-control modal-input" type="text"
						...
						name="preco" required> <input class="form-control modal-input"
						type="text" placeholder="Quantidade (Se não souber digite 'nulo')" name="quantidade" required> <input
